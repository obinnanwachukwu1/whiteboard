...
export const DiscussionDetail: React.FC<Props> = ({ courseId, topicId, title, onBack, onNavigate, isEmbedded, canGoBack }) => {
  ...
  if (isEmbedded) {
    // Single-row titlebar for embedded windows
    return (
      <div className="flex flex-col h-full">
        <div
          className={`flex items-center h-10 ${canGoBack ? '' : 'border-b border-gray-200 dark:border-neutral-700'} bg-white/95 dark:bg-neutral-900/95 shrink-0 app-drag titlebar-right-inset`}
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      >
        ...
        <h2 className="m-0 text-lg font-semibold text-slate-900 dark:text-slate-100 truncate flex-1">
          {topic?.title || title || 'Discussion'}
        </h2>
      </div>
