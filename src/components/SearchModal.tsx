...
export const SearchModal: React.FC<Props> = ({ isOpen, onClose }) => {
      ...
      switch (result.type) {
        ...
        case 'page': {
          const pageId =
            result.pageUrl ||
            result.contentId ||
            (result.id.includes(':') ? result.id.split(':').pop() : '')
          if (pageId) {
            navigate({
              to: '/course/$courseId',
              params: { courseId },
  ...
  const maxIndex = Math.max(
    0,
    results.length + (showDeepSearchAction ? 1 : 0) + (showAskAIAction ? 1 : 0) - 1,
  )
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <span
                    className={`text-[10px] uppercase font-medium px-1.5 py-0.5 rounded ${getTypeBadgeClass(result.type)}`}
                  >
