...
export const FileViewer: React.FC<Props> = ({
  ...
  const name =
    (metaQ.data as any)?.display_name || (metaQ.data as any)?.filename || (metaQ.data as any)?.name
  ...
  const isAudio =
    ['mp3', 'wav', 'ogg', 'm4a', 'aac'].includes(ext) || contentType?.startsWith('audio/')
  const isVideo =
  ...
  const isSupported =
    isPdf || isImage || isAudio || isVideo || isDocx || isDoc || isXlsx || isPptx || isTextLike
  ...
  const renderFallback = (message: React.ReactNode) => (
    <div
      className={`flex flex-col items-center justify-center p-8 h-full text-center ${className}`}
    >
  ...
  if (metaQ.isLoading || fileUrlQ.isLoading) {
    ...
    body = (
      <div className={`p-8 text-slate-500 dark:text-slate-400 ${className}`}>
        Unable to load file metadata.
      </div>
    )
  } else if (isPdf) {
    if (fileUrlQ.isError) {
      body = renderFallback('Failed to load file.')
    } else {
    ...
    if (localUrl) {
      ...
      body = renderFallback('Image not available')
    }
  } else if (isAudio) {
    if (localUrl) {
      body = (
      ...
      <ViewerFrame
        ...
        toolbar={
          <ViewerToolbar
            ...
            disableDownload={!window.system?.downloadFile}
          />
        }
      >
        <div
    ...
    body = renderFallback(`This file type (.${ext}) cannot be viewed directly.`)
  } else if (remoteUrl) {
    body = (
      <ViewerFrame
        className={className}
    ...
    body = (
      <div className={`p-8 text-slate-500 dark:text-neutral-400 ${className}`}>
