...
export default function CoursePage() {
  ...
  const search = useSearch({ from: '/course/$courseId' }) as {
    tab?: string
    type?: string
    contentId?: string
    title?: string
  ...
  const [courseDetail, setCourseDetail] = React.useState<any | null>(
    search?.type && search?.contentId
      ? { contentType: search.type, contentId: search.contentId, title: search.title }
      : null,
  )
  ...
  const onOpenDetail = (d: {
    ...
    navigate({
      to: '/course/$courseId',
      params: { courseId },
      search: { tab: courseTab, type: d.contentType, contentId: d.contentId, title: d.title },
    })
  }
  ...
  const onNavigateCourse = (
    ...
    if (init) {
      ...
      navigate({
        ...
        search: { tab: t, type: init.type, contentId: String(init.id), title: init.title },
      })
    } else {
  ...
  React.useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const tabsData = (queryClient.getQueryData(['course-tabs', id, true]) as any[]) || []
        const infoData = queryClient.getQueryData(['course-info', id]) as any
        const resolvedTabs = computeResolvedTabs(
          infoData || null,
          tabsData,
          hasFilesFromTabs(tabsData),
        )
        const allTabs = resolvedTabs.map((t) => t.key)
